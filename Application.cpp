...
//        scenes.push_back(scene2);
		...
		const std::function<void()> f1 = [p = this]
		{ p->changeScene(0); };
		gui->scene1ButtonDown->Subscribe(new Observer(f1));
		const std::function<void()> f2 = [p = this]
		{ p->changeScene(1); };
		gui->scene2ButtonDown->Subscribe(new Observer(f2));
		const std::function<void()> f3 = [p = this]
		{ p->changeScene(2); };
		gui->scene3ButtonDown->Subscribe(new Observer(f3));
		const std::function<void()> f4 = [p = this]
		{ p->quit(); };
		gui->quitButtonDown->Subscribe(new Observer(f4));
		const std::function<void()> f5 = [p = renderer]
		{ p->setShader(soft3d::FLAT); };
		const std::function<void()> m1 = [p = this]
		{ p->disableMouse(); };
		gui->flatShaderButtonDown->Subscribe(new Observer(f5));
		gui->flatShaderButtonDown->Subscribe(new Observer(m1));
		const std::function<void()> f6 = [p = renderer]
		{ p->setShader(soft3d::GOURAUD); };
		const std::function<void()> m2 = [p = this]
		{ p->disableMouse(); };
		gui->gouraudShaderButtonDown->Subscribe(new Observer(f6));
		gui->gouraudShaderButtonDown->Subscribe(new Observer(m2));
		const std::function<void()> f7 = [p = renderer]
		{ p->setTextureFilter(soft3d::NEIGHBOUR); };
		const std::function<void()> m3 = [p = this]
		{ p->disableMouse(); };
		gui->neighbourButtonDown->Subscribe(new Observer(f7));
		gui->neighbourButtonDown->Subscribe(new Observer(m3));
		const std::function<void()> f8 = [p = renderer]
		{ p->setTextureFilter(soft3d::BILINEAR); };
		const std::function<void()> m4 = [p = this]
		{ p->disableMouse(); };
